defmodule AdventOfCode.E1.P1 do
  @moduledoc """
  ...
  @doc ~S"""
  Formats the raw input for the exercise.

  ## Examples

      iex> input = "-1\n+1"
      iex> AdventOfCode.E1.P1.format(input)
      ["-1", "+1"]

  """
  def format(raw), do: String.split(raw, "\n", trim: true)

  @doc """
  Sums a list of given frequencies.

  ## Examples

      iex> AdventOfCode.E1.P1.sum_frequencies(["-1"])
      -1

      iex> AdventOfCode.E1.P1.sum_frequencies(["-1", "+1"])
      0

      iex> AdventOfCode.E1.P1.sum_frequencies(["-1", "+1"], 1)
      1

  """
  def sum_frequencies(list, count \\ 0)
  def sum_frequencies([], count), do: count

  def sum_frequencies([freq | list], count) do
    count =
      case parse(freq) do
        {:add, n} -> count + n
        {:subtract, n} -> count - n
    ...
    sum_frequencies(list, count)
  end

  @doc """
  Parses a frequency string into a tuple.

  ## Examples

      iex> AdventOfCode.E1.P1.parse("-1")
      {:subtract, 1}

      iex> AdventOfCode.E1.P1.parse("+1")
      {:add, 1}
      
      iex> AdventOfCode.E1.P1.parse("+12")
      {:add, 12}

  """
  def parse("+" <> n), do: {:add, String.to_integer(n)}
  def parse("-" <> n), do: {:subtract, String.to_integer(n)}
  def parse(_), do: {:error, :not_implemented}
end
